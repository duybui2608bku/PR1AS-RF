...
export class VNPayService {
  ...
  buildDepositPaymentUrl(
    ...
    return this.vnpay.buildPaymentUrl(paymentParams);
  }

  verifyPaymentReturn(queryParams: Record<string, string>): VNPayVerifyResult {
    this.ensureCredentials();

    const verifyParams = queryParams as unknown as ReturnQueryFromVNPay;
    const verifyResult = this.vnpay.verifyReturnUrl(verifyParams);
